...
class StrokeAttentionHead_ori(nn.Module):
    '''
    ...
    def __init__(self, stroke_num=256, stroke_dim=13, encoder_embed_dim=768, self_attn_depth=4,control_num=False,num_loss=False):
        super(StrokeAttentionHead_ori, self).__init__()
...
class StrokeAttentionHead(nn.Module):
    ...
    Cross-Attention-based Head
    '''
    def __init__(self, stroke_num=256, stroke_dim=13, encoder_embed_dim=768, self_attn_depth=4,control_num=False,num_loss=False,init=False,hidden_dim=768):
        ...
        if num_loss:
            self.stroke_dim+=1
        self.stroke_tokens = nn.Parameter(torch.randn(1, stroke_dim*stroke_num,hidden_dim))

        if num_loss:
            self.num_stroke_tokens= nn.Parameter(torch.randn(1, 1*stroke_num,hidden_dim))
        else:
            self.num_stroke_tokens=None

        self.cross_attn_block = CrossAttentionBlock(x_dim=encoder_embed_dim, y_dim=hidden_dim, num_heads=min(stroke_num,8))
        self.self_attn_blocks = nn.Sequential(*[
            Block(
    ...
    def forward(self, x,num=None,init_strokes=None,difference_map = None):
        stroke_tokens=self.stroke_tokens
        if self.num_stroke_tokens is not None:
            stroke_tokens = torch.cat([stroke_tokens,self.num_stroke_tokens],dim=1)
        x = self.cross_attn_block(x, stroke_tokens.repeat(x.shape[0],1,1)) #B,stroke_dim*stroke_num,hidden_dim
        x = self.self_attn_blocks(x)  #B,stroke_dim*stroke_num,hidden_dim
        x = self.linear_head(x)  #B,stroke_dim*stroke_num,1
        x=x.reshape(x.size(0),-1,self.stroke_dim)
        return x

#b,stroke_dim,stroke_num
#b,stroke_dim,stroke_num,1
#b,stroke_dim*stroke_num,hiden_dim=768

class StrokeAttentionHead_coarse(nn.Module):
    '''
    Cross-Attention-based Head
    '''
    def __init__(self, stroke_num=256, stroke_dim=13, encoder_embed_dim=768, self_attn_depth=4,control_num=False,num_loss=True):
        super(StrokeAttentionHead_coarse, self).__init__()
        self.control_num=control_num
        if num_loss:
            stroke_dim+=1
        self.stroke_dim=stroke_dim
        if control_num:
            self.control_num_tokens = nn.Parameter(torch.zeros(1, 8, stroke_num))
        self.stroke_tokens = nn.Parameter(torch.zeros(1, stroke_dim, stroke_num))
        self.cross_attn_block = CrossAttentionBlock(x_dim=encoder_embed_dim, y_dim=stroke_num, num_heads=min(stroke_num,8))
        self.self_attn_blocks = nn.Sequential(*[
            Block(
                dim=stroke_num, num_heads=min(stroke_num,8), mlp_ratio=4., qkv_bias=True,
                attn_drop=0., norm_layer=nn.LayerNorm, act_layer=nn.GELU)
            for i in range(self_attn_depth)])
        self.linear_head = nn.Linear(stroke_dim, stroke_dim)
...
class StrokeAttentionPredictor(nn.Module):
    ...
    def __init__(self, stroke_num=512, stroke_dim=13, self_attn_depth=1,control_num=False,num_loss=False,init=False,supersvg_version=False):
        ...
        if num_loss:   #only coarse model has num loss
            self.stroke_head = StrokeAttentionHead_coarse(stroke_num=stroke_num, stroke_dim=stroke_dim,
                                                   ...
                                                   num_loss=num_loss)
        elif supersvg_version:
            self.stroke_head = StrokeAttentionHead_ori(stroke_num=stroke_num, stroke_dim=stroke_dim,
                                                   encoder_embed_dim=self.feature_extractor.embed_dim,
            ...
            self.stroke_head = StrokeAttentionHead(stroke_num=stroke_num, stroke_dim=stroke_dim,
                                                   encoder_embed_dim=self.feature_extractor.embed_dim,
                                                   self_attn_depth=self_attn_depth, control_num=control_num,
                                                   num_loss=num_loss, init=init)
        self.convin=nn.Conv2d
    ...
    def forward(self, x,init_strokes=None,num=None,):
        if x.size(-1) != 224:
            x = self.resize_224(x)
...
class StrokeAttentionPredictor_autoregression(nn.Module):
    '''
    Cross-Attention-based Predictor
    Takes target/canvas inputs and predicts 32 strokes.
    '''
...
class StrokeAttentionPredictor_autoregression2(nn.Module):
    ...
    Cross-Attention-based Predictor
    Takes target/canvas inputs and predicts 32 strokes.
    Combines two encoders in one model.
    '''
    def __init__(self, stroke_num=512, stroke_dim=13, self_attn_depth=1,control_num=False,use_resnet=False,num_loss=False):
        super(StrokeAttentionPredictor_autoregression2, self).__init__()
        self.encoder1=tmp_encoder(0,stroke_num,stroke_dim,self_attn_depth,control_num=control_num,use_resnet=use_resnet,num_loss=num_loss)
        self.encoder2 = tmp_encoder(1, 8, stroke_dim, self_attn_depth,control_num=control_num,use_resnet=use_resnet)
