...
class AttnPainterSVG(nn.Module):
    ...
    def fast_rendering(self, strokes, save_svg_path=None):  # Top-k rendering method.
    ...
    def loss(self, x,mask=None, require_lpips=False,epoch_id=0,num_loss=False,iter_id=0):
        ...
        if iter_id % 80==0:
            with torch.no_grad():
                    save_image(torch.cat(((x*mask)[:8],pred[:8]),dim=0),'tmp_origin_64.jpg',nrow=8)
